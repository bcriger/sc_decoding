from SCLayoutClass import SCLayout
...
class Sim2D(object):
    ...
    def run(self, n_trials, verbose=False, progress=True):
        ...
        return log, x_synd, z_synd, x_corr, z_corr, err
    ...
    def key_from_value(self, dict, value):
        for key, val in dict.iteritems():
            if val == value:
                return key

    def dumb_decoder(self,x_syndrome,z_syndrome):
        '''
        Connects all detection events to the boundary.
        Simple dumb decoder.
        '''
        dumb_correction_z = []
        dumb_correction_x = []
    ...
    def XZ_decomposition(self, Pauli_to_decomp):
        return sp.Pauli(Pauli_to_decomp.x_set,{}), sp.Pauli({}, Pauli_to_decomp.z_set)
