...
test_inputs = np.array(a[:738])
test_targets = np.array(b[:738])
test_inputs1 = np.array(a[:2953])
test_targets1 = np.array(b[:2953])
...
feed_dict_test1 = {x_data: test_inputs1, y_data: test_targets1}
...
filename = 'd_'+str(d) + nodes_str + '_' + str(no_samples) + '_depol'
with tf.Session(config=sess_config) as sess:
    ...
    print(msg2.format(i, sess.run(cost, feed_dict=feed_dict_test), myAccuracy(feed_dict_test)))
    print(msg2.format(i, sess.run(cost, feed_dict=feed_dict_test1), myAccuracy(feed_dict_test1)))
